...
See https://www.pulumi.com/docs/concepts/inputs-outputs for more details.`;
     ...
     * @internal
     */
    static async getPromisedValue(promise, withUnknowns) {
...
async function liftInnerOutput(allResources, value, isKnown, isSecret) {
...
async function applyHelperAsync(allResources, value, isKnown, isSecret, func, runWithUnknowns) {
...
async function getPromisedObject(keysAndOutputs) {
    const result = {};
...
function recover(o, func) {
    ...
    const result = new OutputImpl(new Set(), data.then((d) => d.value), data.then((d) => d.isKnown), data.then((d) => d.isSecret), data.then((d) => d.allResources));
    return result;
}
