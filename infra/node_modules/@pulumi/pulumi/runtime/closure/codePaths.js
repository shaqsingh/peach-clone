...
async function computeCodePathsWorker(options) {
    ...
    const packagePaths = await allFoldersForPackages(new Set(options.extraIncludePackages || []), new Set(options.extraExcludePackages || []), options.logResource);
    ...
    // AssetMap.
    const sourcePaths = new Set(packagePaths.values());
...
async function allFoldersForPackages(includedPackages, excludedPackages, logResource) {
    ...
    for (const pkg of referencedPackages) {
        addPackageAndDependenciesToMap(root, pkg, seenPaths, packagePaths, excludedPackages, 
 ...
 */
function packageArchiveEntry(child) {
 ...
 */
function packageArchivePath(nominalPath, packageName) {
    if (nominalPath.split("/").includes(".pnpm")) {
        return `node_modules/${packageName}`;
    }
    return nominalPath;
}
exports.packageArchivePath = packageArchivePath;
/**
 ...
 * package.
 */
function addPackageAndDependenciesToMap(root, pkg, seenPaths, packagePaths, excludedPackages, dependentPath) {
    ...
    if (nested === undefined) {
        ...
        __1.log.warn(`Cannot include multiple versions of package '${child.name}' at '${childArchivePath}': ` +
    ...
    return;
    function recurse(dependencies, parentArchivePath) {
        if (dependencies) {
            for (const dep of Object.keys(dependencies)) {
                ...
                addPackageAndDependenciesToMap(next, dep, seenPaths, packagePaths, excludedPackages, parentArchivePath);
            }
        }
    }
}
